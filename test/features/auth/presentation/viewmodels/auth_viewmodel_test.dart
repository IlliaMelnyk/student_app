...
void main() {
  ...
  group('AuthViewModel - Přihlašování', () {
    test('login() - správně nastaví stavy a dekóduje JWT token', () async {
      ...
      when(
        () => mockSecureStorage.saveUserEmail(any()),
      ).thenAnswer((_) async {});
      when(
    ...
    test('login() - selže při prázdných polích bez volání API', () async {
      final result = await viewModel.login(
        email: '',
        password: '',
        emptyFieldsText: 'Vyplňte údaje',
        serverErrorText: 'Chyba',
      );

      expect(result, false);
      expect(viewModel.errorMessage, 'Vyplňte údaje');
